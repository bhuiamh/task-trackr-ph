...
const CommonButton = ({ type, href, onClick, className, children }) => {
  ...
  if (href) {
    return (
      <a
        href={href}
        className={`w-full flex items-center rounded-md bg-transparent px-3.5 py-2.5 text-center text-sm font-semibold text-primary shadow-sm hover:bg-primary hover:text-white border-2 border-primary duration-500 ${className}`}
      >
        {children}
      </a>
    );
